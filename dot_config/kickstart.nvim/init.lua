...
-- vim.api.nvim_set_hl(0, 'FloatBorder', { fg = '#1e1e1e', bg = 'none' })
--
...
vim.api.nvim_create_autocmd('BufWritePost', {
  pattern = '*.cs',
  callback = function()
    -- Look upward for the project file
    local godot_project = vim.fs.find('project.godot', { upward = true, stop = vim.loop.os_homedir() })[1]

    if godot_project then
      -- Get the directory containing project.godot
      local project_root = vim.fs.dirname(godot_project)

      vim.fn.jobstart({ 'dotnet', 'build', project_root }, {
        on_exit = function(_, code)
          if code == 0 then
            print 'Godot Build Success!'
          else
...
require('lazy').setup({
      ...
      -- NOTE: `opts = {}` is the same as calling `require('mason').setup({})`
      {
        ...
        opts = {
          registries = {
            ...
            'github:Crashdummyy/mason-registry', -- This one contains the roslyn server
          },
        },
      },
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      -- var of bread in 'food'  and (joints) as well as bread
      -- Examples:
